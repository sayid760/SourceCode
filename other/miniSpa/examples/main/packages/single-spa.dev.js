...
function addErrorHandler(handler) {
  if (typeof handler !== "function") {
    throw Error(
      formatErrorMessage(28, "a single-spa error handler must be a function")
    );
  }
...
function removeErrorHandler(handler) {
  if (typeof handler !== "function") {
    throw Error(
      formatErrorMessage(29, "a single-spa error handler must be a function")
    );
  }
...
function toUnmountPromise(appOrParcel, hardFail) {
  console.log("appOrParcel", appOrParcel);
  console.log("hardFail", hardFail);
  ...
  return Promise.resolve().then(function () {
    ...
    var unmountChildrenParcels = Object.keys(appOrParcel.parcels).map(function (
      parcelId
    ) {
  ...
  loadPromise = loadPromise.then(function (config) {
    ...
    if (
      ...
      throw Error(
        formatErrorMessage(
          9,
          "Parcel ".concat(name, " provided an invalid bootstrap function"),
          name
        )
      );
    }
...
function unloadApplication(appName) {
  ...
  if (typeof appName !== "string") {
    throw Error(
      formatErrorMessage(26, "unloadApplication requires a string 'appName'")
    );
  }
...
function validateRegisterWithConfig(config) {
  if (Array.isArray(config) || config === null)
    throw Error(
      formatErrorMessage(39, "Configuration object can't be an Array or null!")
    );
...
function reroute() {
  ...
  function loadApps() {
    return Promise.resolve().then(function () {
      ...
      var loadPromises = appsToLoad.map(toLoadPromise); // 就是获取到bootstrap,mount和unmount方法放到app上
