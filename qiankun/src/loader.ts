...
export async function loadApp<T extends ObjectType>(
  ...
  console.log(entry, importEntryOpts) // localhost:7100,  {prefetch: true}
  console.log('template', template) // <!DOCTYPE html>...</html>
  console.log('execScripts', execScripts) // ƒ execScripts(proxy,strictGlobal){var execScriptsHooks=arguments.length>2&&arguments[2]!==undefined?arguments[2]:{};if(!scripts.length){return Promise.resolve();}return _execScripts(entry,scripts,proxy…
  console.log('assetPublicPath', assetPublicPath) // http://localhost:7100/

  // 判断是否单例，在单例模式下，新的子应用挂载行为会在旧的子应用卸载之后才开始。
  ...
  "<div id="__qiankun_microapp_wrapper_for_react_16_1626686269789_242__" data-name="react16"><!DOCTYPE html>
    ...
    <div id="root"></div>
    <!--  script http://localhost:7100/static/js/bundle.js replaced by import-html-entry --><!--  script http://localhost:7100/static/js/0.chunk.js replaced by import-html-entry --><!--  script http://localhost:7100/static/js/main.chunk.js replaced by import-html-entry --></body>
    </html>
    </div>"
  */
  ...
  console.log('initialAppWrapperGetter', initialAppWrapperGetter)
